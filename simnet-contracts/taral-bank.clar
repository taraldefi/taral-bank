...
;; validate that the importer / exporter / lender is registered
      ...
      (let 
        ...
        (match interest-transfer-result
          ...
          (begin 
            (let 
              (
                (principal-transfer-result (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.token-susdt transfer outstanding-amount borrower-id lender-id none))
              )
...
;; #[allow(unchecked_params)]
;; #[allow(unchecked_data)]
(define-public (cancel-purchase-order)
  (let 
    (
      (active-purchase-order-id (unwrap! (contract-call? .taral-bank-storage get-active-purchase-order tx-sender) (err ERR_NO_ACTIVE_PURCHASE_ORDER)))
    ...
    (if (is-ok (as-contract (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.token-susdt transfer (get downpayment po) tx-sender borrower-id none)))
        (begin
  ...
  (let 
    ...
    (let ((po-id (get purchase-order-id financing)))
      (let ((po (unwrap! (contract-call? .taral-bank-storage get-purchase-order-by-id po-id) (err ERR_PURCHASE_ORDER_NOT_FOUND))))
        (if (is-ok (contract-call? 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.token-susdt transfer (get financing-amount financing) (as-contract tx-sender) lender-id none))
          (begin
      ...
      (if (not (get refunded financing))
        (begin
          ...
          (try! (as-contract (contract-call? 
                  'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.token-susdt transfer 
