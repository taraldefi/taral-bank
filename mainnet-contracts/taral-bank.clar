...
(define-public (finance (purchase-order-id (string-utf8 36)))
  (let 
    ...
    (if (is-ok (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-susdt transfer loan-amount tx-sender (as-contract tx-sender) none))
        (begin
          (unwrap! (as-contract (contract-call? .taral-bank-storage update-purchase-order purchase-order-id (merge po { 
            updated-at: block-height,
...
(define-public (accept-financing)
  (let 
    ...
    (if (is-ok (as-contract (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-susdt transfer (get financing-amount financing) tx-sender (get borrower-id po) none)))
        (begin
          (if (is-ok (as-contract (contract-call? 'SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0KBR9.token-susdt transfer (get downpayment po) tx-sender (get borrower-id po) none)))
            (begin
